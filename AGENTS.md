...
- Do not introduce a frontend framework, a bundler other than Bun, or a Node
  ...
  frontend is built and shipped with: a package used only by tests or by the
  toolchain is a different question — a browser driver is a reasonable answer to
  one, and `typescript` is the answer to the other. What must not appear is a
  dependency the built `public/` output relies on.
- `typescript` is pinned exactly, not floated. TypeScript 7 is the Go-native
  port and ships no programmatic API, so the Volar-based checkers that read
  Vue/Svelte/Astro templates cannot run on it; 6.0.3 is the newest
  JavaScript-implemented release. Upstream content mappers targeted at 7.1 are
  the condition for lifting the pin (see TQ-0076).
