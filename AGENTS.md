...
- `frontend/markdown.ts` is the only thing in the board that builds HTML, and it
  escapes the document before it emits a tag: there is no sanitiser after it
  because there is nothing left from the file to sanitise. A URL is the
  exception escaping does not cover, so a scheme is either on the allow-list or
  the link stays the text it was written as. Do not reach for a Markdown
  library instead: the built `app.js` is committed, and the two things a library
  would buy — the long tail of CommonMark, and sanitising — are unused by task
  bodies and answered here respectively.
...
since the two are separate implementations of one rule, and `markdown.test.ts`
carries a case for every way a task file could try to become markup, because
that renderer's output is the one string the board puts into the page as HTML.
